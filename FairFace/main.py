...
def detect_face(image_paths,   SAVE_DETECTED_AT, default_max_size=800, size = 300, padding = 0.25):
    ...
    for index, image_path in enumerate(image_paths):
        ...
        for idx, image in enumerate(images):
            ...
            filename = path_sp[0] + "_" + "face" + str(idx) + "." + path_sp[-1]
            face_name = os.path.join(SAVE_DETECTED_AT,  filename)
            dlib.save_image(image, face_name)

    return rects, filename

def predict_age_gender_race(save_prediction_at, bboxes, imgs_path = 'cropped_faces/'):
...
if __name__ == "__main__":
    ...
    imgs = args.input_img
    bboxes, filename = detect_face(imgs, SAVE_DETECTED_AT)
    print("detected faces are saved at ", SAVE_DETECTED_AT)
    predict_age_gender_race("test_outputs.csv", bboxes, SAVE_DETECTED_AT + filename)
